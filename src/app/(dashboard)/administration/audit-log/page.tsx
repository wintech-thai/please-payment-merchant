...
export default function AuditLogPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (searchTerm) {
        if (searchField === 'all') {
          queryMust.push({ multi_match: { query: searchTerm, fields: ['data.userInfo.UserName', 'data.api.ApiName', 'data.api.Controller', 'data.userInfo.Role', 'data.userInfo.IdentityType', 'data.CfClientIp', 'data.ClientIp', 'data.RemoteIp', 'data.Path'], type: 'phrase_prefix' } })
          ...
          queryMust.push({ multi_match: { query: searchTerm, fields: ['data.ClientIp', 'data.CfClientIp', 'data.RemoteIp'], type: 'phrase_prefix' } })
        }
      }
