...
import { Search, RefreshCw, ChevronLeft, ChevronRight, ExternalLink, TriangleAlert, Download } from 'lucide-react'
...
export default function PayOutTransactionsPage() {
  ...
  useOrgChange(() => load(1, itemsPerPage, timeRange, search, statusFilter, p2pFilter))

  const handleSearch = () => {
    setSearch(inputSearch)
    setPage(1)
    load(1, itemsPerPage, timeRange, inputSearch, statusFilter, p2pFilter)
  }
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex-none bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex flex-wrap gap-2 items-center">
        ...
        <select
          value={statusFilter}
          onChange={e => { setStatusFilter(e.target.value); setPage(1); load(1, itemsPerPage, timeRange, search, e.target.value, p2pFilter) }}
        ...
        <select
          value={p2pFilter}
          onChange={e => { setP2pFilter(e.target.value); setPage(1); load(1, itemsPerPage, timeRange, search, statusFilter, e.target.value) }}
          className="text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
        >
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              ...
              <select value={itemsPerPage} onChange={e => {
                const n = Number(e.target.value); setItemsPerPage(n); setPage(1); load(1, n, timeRange, search, statusFilter, p2pFilter)
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => { setPage(p => p - 1); load(page - 1, itemsPerPage, timeRange, search, statusFilter, p2pFilter) }}
      ...
      {exportOpen && (
        <ExportCsvModal<PayOutTxItem>
          onClose={() => setExportOpen(false)}
          filenamePrefix="pay-out-transactions"
          ...
          headers={[
            'Date/Time', 'Amount', 'Currency', 'Fee Amount', 'Fee %', 'Fee Payer',
            'Dest Bank Code', 'Dest Account No', 'Dest Account Name', 'Dest PromptPay ID',
            'Source Bank Code', 'Source Account No', 'Source Account Name', 'Source PromptPay ID',
            'Is P2P', 'Status', 'Status Reason', 'Ref1', 'Ref2', 'Ref3',
          ]}
          mapRow={(item): CsvCell[] => {
            ...
            return [
              ...
              item.payOutFeePct ?? item.payoutFeePct ?? '',
              item.payoutFeePayer ?? '',
              item.payInBankCode ?? '',
              item.payInBankAccountNo ?? '',
              item.payInBankAccountName ?? '',
              item.payInPromptPayId ?? '',
              isKnown(item.payOutBankCode) ?? '',
              isKnown(item.payOutBankAccountNo) ?? '',
              isKnown(item.payOutBankAccountName) ?? '',
          ...
          fetchCount={async params => {
            const res = await paymentTxApi.getPayOutTransactionCount(params as any)
            const d = res.data as any
            return typeof d === 'number' ? d : (d?.count ?? 0)
          }}
          fetchPage={async (params, offset, limit) => {
            const payload = { ...params, offset, limit }
            const res = await paymentTxApi.getPayOutTransactions(payload as any)
            const d = res.data as any
            return Array.isArray(d) ? d : (d?.payOutTransactions ?? d?.payInTransactions ?? d?.items ?? [])
          }}
        />
      )}
    </div>
